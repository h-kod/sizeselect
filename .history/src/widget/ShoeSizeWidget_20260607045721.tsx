...
const TARGET_BRANDS = ['Nike','Adidas','Puma','New Balance']
      ...
      <Drawer open={open} onClose={()=>{setOpen(false); console.log('event','widget_closed')}}>
        <div className="ssw-section">
          <div className="ssw-stepper">
            <span className="ssw-step active">1</span>
            <span className="ssw-step">2</span>
            <span className="ssw-step">3</span>
          </div>
          <label className="ssw-label">Hedef marka</label>
          <div className="ssw-pill-group">
            {TARGET_BRANDS.map(b => (
              <button key={b} type="button" className={`ssw-pill ${selectedTargetBrand===b? 'active':''}`} onClick={()=>setSelectedTargetBrand(b)}>{b}</button>
            ))}
          </div>
          <p className="ssw-section-note">Bu üründe geçerli olan hedef markayı seç; tavsiyeni daha doğru hesaplayalım.</p>
        </div>
